...
import { DEVPLAT_TEMPLATE_LIST_PROMPT_PREFIX, DEVPLAT_TEMPLATE_LIST_PROMPT_SUFFIX, DEVPLAT_RESOLVE_TEMPLATE_PREFIX, TEMPLATE_SEARCH_QUERY_PROMPT, DEVPLAT_RESOLVE_TEMPLATE_SUFFIX } from './constants';
import { searchForTemplate, lookupTemplateByTitle, getTemplateRefToTitleMap, lookupTemplateByTemplateRef, templateToSummary } from './devplat-api';
...
export async function askAgent(messages: vscode.ChatMessage[], token: vscode.CancellationToken, agentName: string = 'copilot') {
	messages.forEach(message => { message.name = AGENT_NAME });
...
export async function askAgentForJson(messages: vscode.ChatMessage[], token: vscode.CancellationToken, agentName: string = 'copilot') {
...
export async function askAgentToFindTemplateList(searchQuery: string, token: vscode.CancellationToken): Promise<TemplateDetail[]> {
	...
	try {
		...
		let incomingJson = await askAgentForJson([
			{
				role: vscode.ChatMessageRole.System,
				content: `${DEVPLAT_TEMPLATE_LIST_PROMPT_PREFIX} ${JSON.stringify(templateSummaryList)} ${DEVPLAT_TEMPLATE_LIST_PROMPT_SUFFIX}`,
			},
			{
				role: vscode.ChatMessageRole.User,
		...
		incomingJson.forEach((incomingResult: any) => {
			...
			if (template.templateRef !== incomingResult.templateRef) {
				outputChannel.appendLine(`Incoming json and result templateRef mismatch. Incoming json: ${incomingResult.templateRef}, result: ${template.templateRef}. Omitting from output.`);
			} else {
...
export async function askAgentToFindOneTemplate(searchQuery: string, token: vscode.CancellationToken): Promise<TemplateDetail | null> {
	...
	// Log if both failed
	outputChannel.appendLine(`Unable to find template using search index. Result count was ${templateList.length} and ${refinedTemplateList.length}. Trying Copilot...`);
	...
	try {
		...
		const outputJson = await askAgentForJson([
			{
				role: vscode.ChatMessageRole.System,
				content: `${DEVPLAT_RESOLVE_TEMPLATE_PREFIX} ${JSON.stringify(inputJson)} ${DEVPLAT_RESOLVE_TEMPLATE_SUFFIX}`,
			},
			{
				role: vscode.ChatMessageRole.User,
