...
import { outputChannel, askAgentForJson } from './common';
...
async function chatAgentHandler(request: vscode.ChatAgentRequest, context: vscode.ChatAgentContext, progress: vscode.Progress<vscode.ChatAgentProgress>, token: vscode.CancellationToken) {
	...
	if (commandRequest.command === AgentCommand.Cancel) {
		...
		progress.report(<vscode.ChatAgentContent>{ content: `Right-o! We can pretend that never happened. What do you want to talk about now?` });
		return {};
	}
	if (commandRequest.command === AgentCommand.None) {
		progress.report(<vscode.ChatAgentContent>{ content: `Hmmm. Not sure what you want me to do. Can you try rephrasing?` });
		return {};
	}
	return await handleDevPlatformApiChatCommand(agentState, commandRequest, progress, token);
}

async function resolveCommand(request: vscode.ChatAgentRequest, token: vscode.CancellationToken): Promise<CommandRequest> {
	const promptContent = request.prompt.trim();
	if (request.slashCommand?.name === 'cancel' || promptContent.startsWith('/abort') || promptContent.startsWith('/cancel')) {
	...
	if (agentState.commandAlreadyInProgress !== AgentCommand.FindCommand && agentState.commandAlreadyInProgress !== AgentCommand.None) {
		return <CommandRequest>{
