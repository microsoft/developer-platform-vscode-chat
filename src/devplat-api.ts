...
export async function callApi(apiPath: string, method: string = 'GET', bodyJson: any = null): Promise<DevPlatApiResult> {
...
async function createTemplateSearchIndex() {
	...
	templates.forEach((template: any, idx: number) => {
		...
		const indexObj = <any>{
			...
			tagString: templateMetadata.tags ? templateMetadata.tags.reduce((acc: string, tag: string) => `${acc} "${tag}"`) : '',
			createsString: template.spec?.creates ? templateMetadata.spec?.creates.reduce((acc: string, obj: any) => `${acc} ${obj.kind}`, 'creates ') : ''
		}
...
export async function searchForTemplate(query: string, exact = false, exactFields: string[] | null = null): Promise<TemplateDetail[]> {
